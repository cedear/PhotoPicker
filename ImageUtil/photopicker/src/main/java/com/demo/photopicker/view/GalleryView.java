...
import com.demo.photopicker.download.GalleryDownLoadCallBack;
import com.demo.photopicker.download.GalleryDownloadThread;
...
public class GalleryView extends RelativeLayout {
    ...
    private static final int DEFAULT_ANIM_DURATION = 200; // 毫秒
    ...
    private List<GalleryPhotoInterface> photoList;
    ...
    public void setGalleryViewFadedListener(OnGalleryViewFadedListener listener) {
        galleryViewFadedListener = listener;
    }
     ...
     */
    public void showPhotoGallery(int index, List<? extends GalleryPhotoInterface> photoList, ImageView clickImageView) {
        GalleryPhotoParameterModel photoParameter = new GalleryPhotoParameterModel();
        //图片地址
        photoParameter.photoObj = photoList.get(index).getPhotoResource();
        //图片在list中的索引
    ...
    public void showPhotoGallery(List<? extends GalleryPhotoInterface> photoList, GalleryPhotoParameterModel photoParameter) {
        //将第一个点击的图片参数连同整个图片列表传入
        this.setVisibility(View.VISIBLE);
        post(new Runnable() {
            @Override
    ...
    private void setGalleryPhotoList(List<? extends GalleryPhotoInterface> list, GalleryPhotoParameterModel parameterModel) {
//        boolean isFocus = requestFocus();
    ...
    private void initView() {
        View view = View.inflate(getContext(), R.layout.photo_picker_gallery_view_main, this);
    ...
    private void saveBitmapToLocation() {
        GalleryDownloadThread download = new GalleryDownloadThread(getContext(), photoList.get(position).getPhotoResource(), new GalleryDownLoadCallBack() {
            @Override
    ...
    private void calculateScaleAndStartZoomOutAnim() {
        ...
        executeZoom(mScaleImageView, translationX, translationY, scale, false, new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mScaleImageView.setVisibility(GONE);
                setVisibility(GONE);
                if (galleryViewFadedListener != null) {
                    galleryViewFadedListener.onGalleryViewFaded();
                }
            }
    ...
    public interface OnGalleryViewFadedListener {
        void onGalleryViewFaded();
    }
